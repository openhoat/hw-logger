...
that = {
  ...
  init: function (options) {
    ...
    if (!that.config.format) {
      that.config.formatFile = path.resolve(path.join(__dirname, '..', 'templates'), that.config.formatFile || 'default.ejs');
      if (!path.extname(that.config.formatFile)) {
        that.config.formatFile = that.config.formatFile + '.ejs';
      }
